...
const _scx = -0.1, _scz = 0.61; // スコアパネル中心（ローカル）
        ...
        return PLAYERS.map((p, seatIdx) => {
          ...
          return (
            <group key={`wind-${seatIdx}`} rotation={[0, p.rotY, 0]}>
              ...
              {/* 持ち点パネル（凹み） */}
              <mesh rotation={[-Math.PI / 2, 0, 0]} position={[-0.1, 0.005, 0.61]} geometry={scorePanelGeom}>
                <meshStandardMaterial color="#030303" roughness={0.95} metalness={0} />
              </mesh>
              {/* 持ち点 */}
              <Text
                font={FONT_DIGI}
                position={[-0.33, 0.009, 0.6]}
                rotation={[-Math.PI / 2, 0, 0]}
                fontSize={0.095}
                color="#ff3333"
                anchorX="left"
