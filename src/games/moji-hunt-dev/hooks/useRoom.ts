...
const cleanupOldRooms = async () => {
  try {
    const roomsRef = ref(db, 'moji-hunt-dev-rooms');
    ...
    for (const [code, room] of Object.entries(rooms)) {
      ...
      if (now - createdAt > maxAge || playerCount === 0) {
        deletePromises.push(remove(ref(db, `moji-hunt-dev-rooms/${code}`)));
      }
    }
...
export const useRoom = (playerId: string | null, playerName: string | null) => {
  ...
  const createRoom = useCallback(async () => {
    ...
    try {
      ...
      const roomRef = ref(db, `moji-hunt-dev-rooms/${code}`);
  ...
  const joinRoom = useCallback(async (code: string) => {
    ...
    try {
      ...
      const roomRef = ref(db, `moji-hunt-dev-rooms/${upperCode}`);
      ...
      return new Promise<boolean>((resolve) => {
        onValue(roomRef, async (snapshot) => {
          ...
          await update(ref(db, `moji-hunt-dev-rooms/${upperCode}/gameState`), {
  ...
  const leaveRoom = useCallback(async () => {
    ...
    try {
      ...
      if (isHost) {
        // ホストの場合はルームを削除
        await remove(ref(db, `moji-hunt-dev-rooms/${roomCode}`));
      } else {
        // プレイヤーの場合は自分を削除
  ...
  const updateGameState = useCallback(async (newState: Partial<GameState>) => {
    ...
    try {
      await update(ref(db, `moji-hunt-dev-rooms/${roomCode}/gameState`), newState);
    } catch (err) {
