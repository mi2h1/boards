...
const cleanupOldRooms = async () => {
  try {
    const roomsRef = ref(db, 'jackal-dev-rooms');
    ...
    for (const [code, room] of Object.entries(rooms)) {
      ...
      if (now - createdAt > maxAge || playerCount === 0) {
        deletePromises.push(remove(ref(db, `jackal-dev-rooms/${code}`)));
      }
    }
...
export const useRoom = (playerId: string | null, playerName: string | null) => {
  ...
  useEffect(() => {
    ...
    const roomRef = ref(db, `jackal-dev-rooms/${roomCode}`);
    const myPresenceRef = ref(db, `jackal-dev-rooms/${roomCode}/presence/${playerId}`);
  ...
  useEffect(() => {
    ...
    const presenceListRef = ref(db, `jackal-dev-rooms/${roomCode}/presence`);
    const roomRef = ref(db, `jackal-dev-rooms/${roomCode}`);
  ...
  const createRoom = useCallback(async () => {
    ...
    try {
      ...
      const roomRef = ref(db, `jackal-dev-rooms/${code}`);
  ...
  const joinRoom = useCallback(async (code: string) => {
    ...
    try {
      ...
      const roomRef = ref(db, `jackal-dev-rooms/${upperCode}`);
      ...
      return new Promise<boolean>((resolve) => {
        onValue(roomRef, async (snapshot) => {
          ...
          await update(ref(db, `jackal-dev-rooms/${upperCode}/gameState`), {
  ...
  const leaveRoom = useCallback(async () => {
    ...
    try {
      ...
      if (isHost) {
        await remove(ref(db, `jackal-dev-rooms/${roomCode}`));
      } else {
        ...
        if (updatedPlayers.length === 0) {
          ...
          await update(ref(db, `jackal-dev-rooms/${roomCode}/gameState`), {
            players: updatedPlayers,
          });
        }
      }
  ...
  const updateGameState = useCallback(async (newState: Partial<GameState>) => {
    ...
    try {
      await update(ref(db, `jackal-dev-rooms/${roomCode}/gameState`), newState);
    } catch (err) {
