...
> Claude Code 引き継ぎ仕様書 v2.1（2026-02-23 更新）
...
| 8. 次の手番       | 反時計回り（右→対面→左→自分）で次プレイヤーへ              |
...
| 要素         | 実装                                                                              |
|--------------|-----------------------------------------------------------------------------------|
| カメラ       | PerspectiveCamera `[0,6,5]` FOV=35, target=`[0,0,0.4]`（固定・操作不可）       |
...
- 2段目以降は手前方向に配置（`RIVER_Z + row * RIVER_ROW_SPACING`、間隔=`TILE_H + 0.02`）
...
### 3D Canvas
- TableScene: 卓・牌・中央パネル
- カメラ固定（ドラッグ・スクロールによる視点変更不可）

### 配牌アニメーション
- 局開始時、全プレイヤーの河が空（新規局）の場合に発動
- 120ms間隔で1枚ずつラウンドロビン配牌（自家→右→対面→左→自家→…）
- 配牌完了まで操作・ゲームループを抑止（`isDealing` ガード）

### 待ち牌パネル（2Dオーバーレイ）
- テンパイ（手牌5枚）時に手牌の上に表示
- 黒背景パネル + 「待ち」ラベル + 牌画像
- 牌画像は TileModel と同じテクスチャパスの `<img>` タグで表示
- **ホバープレビュー**: 手牌6枚時、手牌にマウスオンすると、その牌を切った場合の待ち牌を表示
- 結果モーダル・配牌演出中は非表示

### 操作ボタン（2Dチケット風）
- CSS mask で四隅凹みノッチ + 内側ボーダー（`::after`）
- フォント: YujiSyuku（局数表示と統一）
- 配置: 手牌右上あたり（`bottom-[34%] right-[18%]`）
- ツモボタン（赤）: 自分の手番 + ツモ和了可能時に表示
- ロンボタン（赤）+ 見逃しボタン（灰）: ロン可能時に表示
- 手牌クリック: 打牌フェーズ（手牌6枚）時にクリックで打牌

### ホバー演出
- 打牌可能な手牌にホバーすると Y+0.05 浮き上がり + カーソルpointer
- 6枚手牌ホバー時は待ち牌パネルにプレビュー表示
...
| `GameScreen` | ゲームループ（draw→discard→ron_check）、Bot自動プレイ、持ち時間管理、待ち牌2Dパネル、ホバープレビュー、2Dアクションボタン、結果モーダルオーバーレイ、配牌アニメーション |
| `TableScene` | 3Dシーン全体（卓・牌配置・中央パネル・カメラ固定） |
